...
//	uint32_t th_e = 1000;
	...
	coldisc_th_t coldisc_th = {50,200,100,42,84,126,168,210,252};
	coldisc_th_t coldisc_th2;

	uint8_t exp_th = 60; //127;
	uint32_t exp_pos;
	uint32_t exp_neg;
...
//	data      = img->imageData;
	...
	if (argc>1)
		...
		case 1:
			...
			break;
		case 2:
			vdma_addr = VDMA2_BASEADDR;
			id = 2;
			printf("colseg image\n");
			coldisc_th_write(&coldisc_th);
//			sleep(0.1);
//			coldisc_th_read(&coldisc_th2);
//			coldisc_th_print(&coldisc_th2);
			w = w/4;
			break;
//		case 3:
...
//	if (init_vasa_socket_tcp () == -1) return -1;
