...
public:
  ...
  MonoOdometer(const std::string& transport) : OdometerBase(), replace_(false)
  {
    ...
    camera_sub_ = it.subscribeCamera("cv_camera/image_rect_color", 1, &MonoOdometer::imageCallback, this, transport);

    info_pub_ = local_nh.advertise<VisoInfo>("cv_camera/camera_info", 1);
  }
